...
" {{{ *.md link handler via sermit

" run 'bat --list-thees' to see options
let g:vimwiki_markdown_bat_options = (
...
" sermit & markserve planned, but the user must be aware they need to stop the
" server when finished checking the file. A command is possible, but..
" undecided on the validity of the idea.
let g:vimwiki_markdown_link_handlers = (
      \ "bat",
      \ "cat",
      \ )
...
function! VimwikiLinkHandler(link) abort
  let s:link_markup = a:link
  let s:exec_cmd = "cat"
  let s:exec_action = "bufnew"

  " if s:link_markup !~# '^sermit:'
    " let s:exec_cmd = "sermit"
  " elseif s:link_markup !~# '^markserv:'
    " let s:exec_cmd = "markserv"
  if s:link_markup !~# '^bat:'
    let s:exec_cmd = "bat " . join(g:vimwiki_markdown_bat_options, ' ')
  endif

  let s:link = s:link_markup[1:]
  let s:link_info = vimwiki#base#resolve_link(s:link)

  if empty(s:link_info.filename)
    call maktaba#error#Shout('File does not exist: ' . s:link)
    return 0
  endif

  let s:exec_name = fnameescape(s:link_infos.filename)
  let s:exec_path = getcwd()
  let s:cmd = s:exec_cmd . ' ' . s:exec_name

  if s:exec_action == "bufnew"
    exec c
  else
    " exec s:exec_name
    call maktaba#error#Shout('No valid render method available for ' . s:link)
    return 0
  end
endfunction
